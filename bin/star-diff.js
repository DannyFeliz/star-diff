...
function rateLimitReached() {
  ...
  log(
    colors.red(
      `You reached the call limit per minute to the Github search API. ` +
...
async function fetchRepoData(repo) {
  ...
  let resultRepo = await axios({ params: { q: repo } })
    ...
    .catch((error, otro, param) => {
      ...
      log(
        colors.yellow(
          `You will be able to make requests again ${agoTime} (${formatedTime})`
        )
      );
      hasError = true;
    });
...
async function getRepoData(repo1, repo2) {
  log(
    colors.yellow(
      "Hint: If you want to be more especify you can add the full name of the repository. E.g. vuejs/vue"
    )
  );
